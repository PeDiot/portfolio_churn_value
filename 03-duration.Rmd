...
\begin{equation}
  \frac{\lambda(t|\pmb{x_i})}{\lambda(t|\pmb{x_k})} = \frac{\phi(\pmb{x_i}, \beta) }{\phi(\pmb{x_j}, \beta)}
  (\#eq:prophazratio)
\end{equation}
...
In *proportional hazards* models, the marginal effect of covariate $x_j$ on the hazard function can be easily derived since this computation only requires knowledge on $\beta$. As shown in @CAMERON_TRIVEDI, a one-unit increase in the $j^{\text{th}}$ covariate leads to the following variation in the hazard function *ceteris paribus*: 

\begin{equation}
    \frac{\partial \lambda(t|\pmb{\mathrm{x}}, \beta)}{\partial x_j} = \lambda(t|\pmb{\mathrm{x}}, \beta) \frac{\partial \phi(\pmb{\mathrm{x}}, \beta) / \partial x_j}{\phi(\pmb{\mathrm{x}}, \beta) }
    (\#eq:meph)
\end{equation}
...
The contribution of a spell in $D(t_j)$ to the likelihood function equals the conditional probability that the spell ends at $t_j$ given it is exposed at that specific time (see @CAMERON_TRIVEDI for more details) and can be written as: 
...
The latter formulation of the partial likelihood function is explained in more details in equations \@ref(eq:partlikproof1) and (\#eq:partlikproof2) in the appendix. 
