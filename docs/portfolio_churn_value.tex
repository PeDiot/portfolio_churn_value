...
As shown by table \ref{tab:dataoverview}, the \texttt{Churn\_Value} status variable indicates whether the customer left the firm's \emph{portfolio} within the last month and \texttt{Tenure\_Months} is the duration variable. The two features are used as response variables in the survival models.

\hypertarget{target-variables}{%
\section{Target Variables}\label{target-variables}}

Since the purpose of our study relies in estimating the overall value of this fictional firm's \emph{portfolio}, two groups of target variables can be considered. On the one hand \texttt{Churn\_Value} and \texttt{Tenure\_Months} permit to determine whether a customer is active in the \emph{portfolio}. On the other hand, the \texttt{CLTV} variable represents each customer's \emph{value} through measurement of customer lifetime value.

\hypertarget{churn_value-and-tenure_months}{%
\subsection*{\texorpdfstring{\texttt{Churn\_Value} and \texttt{Tenure\_Months}}{Churn\_Value and Tenure\_Months}}\label{churn_value-and-tenure_months}}
\addcontentsline{toc}{subsection}{\texttt{Churn\_Value} and \texttt{Tenure\_Months}}
...
\caption{Survival depending on demographic information}\label{fig:kmdemographics}
\end{figure}
...
\caption{Survival depending on services information}\label{fig:kmservices}
\end{figure}
...
\caption{Survival depending on customer account information}\label{fig:kmcustaccount}
\end{figure}

\hypertarget{cltv}{%
\subsection{\texorpdfstring{\texttt{CLTV}}{CLTV}}\label{cltv}}
